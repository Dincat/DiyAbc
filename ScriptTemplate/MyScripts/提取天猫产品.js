...
function get_Infor() {
    ...
    try {
        ...
        if (window.location.href.indexOf('tmall.com') < 0 && window.location.href.indexOf('liangxinyao.com') < 0) {
            return "商品页面地址不正确或脚本选择错误";
        }


        //商品名称
        var store_name = document.getElementsByClassName('tb-detail-hd')[0]['firstElementChild'].innerText;


        //商品简介
        var store_info = document.getElementsByClassName('newp')[0];
        if (store_info) {
        ...
        //获取商品售价
        var str_price = document.getElementsByClassName('tm-price')[1];

        if (!str_price) {
            str_price = document.getElementsByClassName('tm-price')[0];
        }
        ...
        //获取商品原价
        var str_ot_price = document.getElementsByClassName('tm-price')[0];
        if (str_ot_price) {
        ...
        //获取商品详情
        var info = document.getElementsByClassName('ke-post')[0];
        ...
        imgs.forEach(function (img) {
            ...
            if (src.indexOf('imglazyload/spaceball.gif') > 0) {
                img.parentElement.removeChild(img);
                return;
            }
        ...
        var maps = info.querySelectorAll("map");
        if (maps) {
            maps.forEach(function (item) {
                var img = item.previousSibling;
                if (img) {
                    img.parentElement.removeChild(img);
                }
        ...
        var divs = info.querySelectorAll("div");
        if (divs) {
        ...
        var adImages = document.getElementsByClassName('tb-thumb')[0];

        if (adImages) {
            adImages = adImages.querySelectorAll("img");
        ...
        if (video_src) {
            ...
            video_src = mycmds.UploadVideo(video_src, dir, md5 + extName);
        }
        ...
        td2.append("creator_id", mycmds.getLoginUserID());
        td2.append("creator_type", 1);
        td2.append("creator_name", mycmds.getLoginUserName());
        td2.append("puid", mycmds.getLoginUserID());
        td2.append("pnickname", mycmds.getLoginUserName());
        td2.append("ficti", random(100, 10000));
        td2.append("stock", random(1000, 10000));
        td2.append("from_source", "天猫");
